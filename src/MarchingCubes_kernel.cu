...
__device__
float sampleVolume_2(float *data, uint3 p, uint3 gridSize, float f_val)
{
    p.x = min(p.x, gridSize.x);
    p.y = min(p.y, gridSize.y);
    p.z = min(p.z, gridSize.z);
...
__global__ void
classifyVoxel(uint *voxelVerts, uint *voxelOccupied, float *volume_one,float *volume_two,
...
void MarchingCubeCuda::classifyVoxel_lattice(dim3 grid, dim3 threads, uint *voxelVerts, uint *voxelOccupied, float *volume_one,float *volume_two,
                     uint3 gridSize, uint3 gridSizeShift, uint3 gridSizeMask, uint numVoxels,
                     float3 voxelSize, float isoValue)
{

 
    classifyVoxel<<<grid, threads>>>(voxelVerts, voxelOccupied, volume_one,volume_two,
...
generateTriangles_lattice_kernel(float4 *pos, float4 *norm, uint *compactedVoxelArray, uint *numVertsScanned, float *volume,
                   ...
                   cudaTextureObject_t triTex, cudaTextureObject_t numVertsTex,uint totalverts, float *volume_one,float *volume_two,float isoValue1, float iso1, bool retain)
{
    ...
    if (i < activeVoxels)
    {
        ...
        else if(cubeindex == 0)
        {
            ...
            cubeindex =  uint(field_2[0] < isoVal);
            cubeindex += uint(field_2[1] < isoVal)*2;
            cubeindex += uint(field_2[2] < isoVal)*4;
            cubeindex += uint(field_2[3] < isoVal)*8;
            cubeindex += uint(field_2[4] < isoVal)*16;
            cubeindex += uint(field_2[5] < isoVal)*32;
            cubeindex += uint(field_2[6] < isoVal)*64;
            cubeindex += uint(field_2[7] < isoVal)*128;

            numVerts = tex1Dfetch<uint>(numVertsTex, cubeindex);

            __shared__ float3 vertlist[12*NTHREADS];
            vertlist[threadIdx.x] = vertexInterp4(isoValue1, v[0], v[1], field_2[0], field_2[1]);
            vertlist[NTHREADS+threadIdx.x] = vertexInterp4(isoValue1, v[1], v[2], field_2[1], field_2[2]);
            vertlist[(NTHREADS*2)+threadIdx.x] = vertexInterp4(isoValue1, v[2], v[3], field_2[2], field_2[3]);
            vertlist[(NTHREADS*3)+threadIdx.x] = vertexInterp4(isoValue1, v[3], v[0], field_2[3], field_2[0]);
            vertlist[(NTHREADS*4)+threadIdx.x] = vertexInterp4(isoValue1, v[4], v[5], field_2[4], field_2[5]);
            vertlist[(NTHREADS*5)+threadIdx.x] = vertexInterp4(isoValue1, v[5], v[6], field_2[5], field_2[6]);
            vertlist[(NTHREADS*6)+threadIdx.x] = vertexInterp4(isoValue1, v[6], v[7], field_2[6], field_2[7]);
            vertlist[(NTHREADS*7)+threadIdx.x] = vertexInterp4(isoValue1, v[7], v[4], field_2[7], field_2[4]);
            vertlist[(NTHREADS*8)+threadIdx.x] = vertexInterp4(isoValue1, v[0], v[4], field_2[0], field_2[4]);
            vertlist[(NTHREADS*9)+threadIdx.x] = vertexInterp4(isoValue1, v[1], v[5], field_2[1], field_2[5]);
            vertlist[(NTHREADS*10)+threadIdx.x] = vertexInterp4(isoValue1, v[2], v[6], field_2[2], field_2[6]);
            vertlist[(NTHREADS*11)+threadIdx.x] = vertexInterp4(isoValue1, v[3], v[7], field_2[3], field_2[7]);
...
void MarchingCubeCuda::generateTriangles_lattice(dim3 grid, dim3 threads,
                          ...
                          uint3 gridSize, uint3 gridSizeShift, uint3 gridSizeMask,
                          float3 voxelSize, float3 gridcenter, float isoValue, uint activeVoxels, uint maxVerts, uint totalverts,float *volume_one,float *volume_two,float isovalue1,float iso1, bool retain)
{
    ...
    generateTriangles_lattice_kernel<<<grid, threads>>>(pos, norm,
                                           ...
                                           maxVerts, triTex_s, numVertsTex_s, totalverts, volume_one,volume_two,isovalue1,iso1,retain);
    cudaDeviceSynchronize();
